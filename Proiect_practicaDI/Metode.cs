...
namespace Proiect_practicaDI
{
    public static class Metode
    {
        ...
        private static void StartMessagePolling()
        {
            ...
            messageTimer = new Timer(500); // Interval de 500 ms
            messageTimer.Elapsed += OnMessageTimerElapsed;
            messageTimer.AutoReset = true; // Ciclare automată
            messageTimer.Enabled = true; // Pornește timer-ul
        }
        private static void OnMessageTimerElapsed(object source, ElapsedEventArgs e)
        {
            string com="AT+CMGL=";
            ...
            foreach (string line in lines)
            {
                if (line != "\n"&& line != "OK")
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(line)&& line.Trim() != "OK")
                        {
        ...
        private static string ExtractPhoneNumberFromParts(string[] parts)
        {
            ...
            if (parts.Length >= 3)
            {
                string phoneNumberPart = parts[2].Trim('"'); // Poate fi nevoie să adaptezi indexul
